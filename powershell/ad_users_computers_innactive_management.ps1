<# Created By ::          shadexic
   ...
   Modified On ::         2021-07-14
   Modification Note ::   Added Disable Feature, Computer functionality, and preped script for review and deployment.
   Version ::             1.3
   Github ::              github.com/shadexic
   Notes ::               Remove the -Whatif Flag once you have tested this script
                          (by checking the $report array after its run)
...
#>
<#
Commands used to help with writing script:
...
if ( $inactivecomputers.Count -gt 0) {
    ...
    $report += [pscustomobject]@{ObjectType="Machine";LastLogonDate=$machine.LastLogonDate;Date=$reporttimestamp;Identifier=$machine.SamAccountName;EnabledStatus=$machine.Enabled;DistinguishedName=$user.DistinguishedName}
    }
}
#generate a "blank" entry in the logfile for visual aid in parsing the log
